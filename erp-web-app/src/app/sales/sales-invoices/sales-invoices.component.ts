...
export class SalesInvoicesComponent implements OnInit {
    ...
    this.menuItems = [
      {
        label: 'New', icon: 'pi pi-file', command: () => {
          this.router.navigate(['/sales/sales-invoice', 0]);
        }
      },
      {
  ...
  getData(): any {
    ...
    var salesOrdersRequest = this.salesService.querySalesInvoices();
      ...
      .subscribe((response) => {
        ...
        _.forEach(salesOrdersResponse, (record => {
          ...
          this.orders.push({
            ...
            customer: customer != null ? customer.name : '', systemNo: record.systemNo, refNo: record.refNo,

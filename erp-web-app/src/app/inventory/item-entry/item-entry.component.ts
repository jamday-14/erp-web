...
export class ItemEntryComponent implements AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.app.title = "Item Entry - Entry";
  }
  ...
  private initializeHeader() {
    if (!this.newItem) {
      this.loading = true;
      var detailRequest = this.inventoryService.queryItemEntryDetails(this.id);
      var headerRequest = this.inventoryService.getItemEntry(this.id);
  ...
  initializeDetails(arr: Array<any>): any {
    ...
    if (arr == null)
      ...
      _.forEach(arr, (record => {
        ...
        this.details.push({
          index: _.size(this.details), id: record.id, itemEntryId: record.itemEntryId, qtyOnHand: record.qtyOnHand,
  ...
  private addTableRow() {
    this.details.unshift({
      ...
      index: _.size(this.details), id: null, itemEntryId: null, qtyOnHand: 0,
  ...
  onDeleteRow(selectedRows: Array<any>) {
    ...
    var nonApiCallsRequest = selectedRows.filter(x => x.itemEntryId == null);
    ...
    var apiCallsRequest = selectedRows.filter(x => x.itemEntryId != null);
    if (apiCallsRequest != null && apiCallsRequest != []) {
      var deleteRequests = [];
      _.forEach(apiCallsRequest, (row => {
        deleteRequests.push(this.inventoryService.deleteItemEntryDetail(row.id, row.itemEntryId));
      }));
      this.loading = true;
    ...
    headerRequest.subscribe((resp) => {
      ...
      _.forEach(this.inventoryDetailComponent.getDetails(), (detail) => {
        ...
        if (detail.id == null)
          detailsRequests.push(
            this.inventoryService.addItemEntryDetail(
              controls.warehouseId.value,
              {
                itemEntryId: entry.id,
          ...
          detailsRequests.push(
            this.inventoryService.updateItemEntryDetail({
              id: detail.id,
              itemEntryId: detail.id,
              itemId: detail.itemId,
              qty: detail.qty,
              unitId: detail.unitId,
              reasonId: detail.reasonId,
      ...
      forkJoin(detailsRequests)
        .subscribe((resp) => {
          ...
          setTimeout(() => {
            this.router.navigate(['/inventory/item-entries']);
          }, 1000)
